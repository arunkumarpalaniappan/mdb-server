...
const url = `mongodb://${config.get('databaseConfig.user')}:${config.get('databaseConfig.pwd')}@${config.get('databaseConfig.host')}:${config.get('databaseConfig.port')}/movies`; //create mongo url
let db, collection;
...
exports.list = function () {
    ...
    return new Promise((resolve, reject) => {
        const cursor = collection.find({}).project({ "_id": true, "original_title": true, "poster_path": true, "spoken_languages": true, "genres": true, "release_date": true }).sort({ $natural: 1 }).skip((parseInt(this.p) - 1) * 42).limit(42); // sort,skip first n and get the response
